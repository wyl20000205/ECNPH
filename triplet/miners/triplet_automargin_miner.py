...
from triplet.miners.triplet_margin_miner import TripletMarginMiner


def to_cpu(x: torch.Tensor, required_grad=False, use_numpy=True):
...
class TripletAutoParamsMiner(TripletMarginMiner):
    ...
    def get_matches_and_diffs_onehot(self, labels, ref_labels=None):
        if ref_labels is None:
            ref_labels = labels
        # print(type(labels),type(ref_labels))
        matches = (labels.float() @ ref_labels.T.float() > 0).byte()
        diffs = matches ^ 1
